...
function watchPostEffect(effect, options) {
    return doWatch(effect, null, (("production" !== 'production')
...
function withDirectives(vnode, directives) {
    ...
    if (internalInstance === null) {
        ("production" !== 'production') && warn(`withDirectives can only be used inside render functions.`);
        return vnode;
    }
...
function applyOptions(instance) {
    ...
    if (dataOptions) {
        ...
        if (!isObject(data)) {
            ("production" !== 'production') && warn(`data() should return an object.`);
        }
        else {
...
function createAppAPI(render, hydrate) {
    return function createApp(rootComponent, rootProps = null) {
        ...
        if (rootProps != null && !isObject(rootProps)) {
            ("production" !== 'production') && warn(`root props passed to app.mount() must be an object.`);
            rootProps = null;
        }
        ...
        const app = (context.app = {
            ...
            use(plugin, ...options) {
                if (installedPlugins.has(plugin)) {
                    ("production" !== 'production') && warn(`Plugin has already been applied to target app.`);
                }
...
function createHydrationFunctions(rendererInternals) {
    ...
    const hydrateElement = (el, vnode, parentComponent, parentSuspense, slotScopeIds, optimized) => {
        ...
        if (("production" !== 'production') || forcePatchValue || patchFlag !== -1 /* HOISTED */) {
            if (dirs) {
            ...
            : isTeleport(type)
                ...
                : isObject(type)
                    ...
                    : isFunction(type)
                        ...
                        : 0;
    if (("production" !== 'production') && shapeFlag & 4 /* STATEFUL_COMPONENT */ && isProxy(type)) {
        type = toRaw(type);
...
function cloneVNode(vnode, extraProps, mergeRef = false) {
    ...
    const cloned = {
        ...
        children: ("production" !== 'production') && patchFlag === -1 /* HOISTED */ && isArray(children)
...
function setupStatefulComponent(instance, isSSR) {
    ...
    if (setup) {
        ...
        setCurrentInstance(instance);
        pauseTracking();
        const setupResult = callWithErrorHandling(setup, instance, 0 /* SETUP_FUNCTION */, [("production" !== 'production') ? shallowReadonly(instance.props) : instance.props, setupContext]);
        resetTracking();
        unsetCurrentInstance();
...
function createSetupContext(instance) {
    const expose = exposed => {
        if (("production" !== 'production') && instance.exposed) {
...
const piniaSymbol = (("production" !== 'production') ? Symbol('pinia') : /* istanbul ignore next */ Symbol());
...
function createPinia() {
    ...
    const pinia = markRaw({
        install(app) {
            ...
            if (!isVue2) {
                ...
                /* istanbul ignore else */
                if (("production" !== 'production') && IS_CLIENT) {
    ...
    if (("production" !== 'production') && IS_CLIENT && !("production" === 'test')) {
        pinia.use(devtoolsPlugin);
    }
    return pinia;
}
...
/**
 * Creates an _accept_ function to pass to `false` in Vite applications.
 *
 * @example
 * ```js
 * const useUser = defineStore(...)
 * if (false) {
 *   false.accept(acceptHMRUpdate(useUser, false))
 * }
 * ```
 *
...
const skipHydrateSymbol = ("production" !== 'production')
...
function createOptionsStore(id, options, pinia, hot) {
    ...
    let store;
    function setup() {
        if (!initialState && (!("production" !== 'production') || !hot)) {
            /* istanbul ignore if */
            if (isVue2) {
        ...
        const localState = ("production" !== 'production') && hot
...
function createSetupStore($id, setup, options = {}, pinia, hot, isOptionsStore) {
    ...
    if (("production" !== 'production') && !pinia._e.active) {
    ...
    /* istanbul ignore else */
    if (("production" !== 'production') && !isVue2) {
    ...
    // by the setup
    if (!isOptionsStore && !initialState && (!("production" !== 'production') || !hot)) {
        /* istanbul ignore if */
        if (isVue2) {
    ...
    /* istanbul ignore next */
    const $reset = ("production" !== 'production')
        ? () => {
    ...
    const store = reactive(assign(("production" !== 'production') && IS_CLIENT
    ...
    for (const key in setupStore) {
        ...
        if ((isRef(prop) && !isComputed(prop)) || isReactive(prop)) {
            ...
            const actionValue = ("production" !== 'production') && hot ? prop : wrapAction(key, prop);
    ...
    Object.defineProperty(store, '$state', {
        get: () => (("production" !== 'production') && hot ? hotState.value : pinia.state.value[$id]),
        set: (state) => {
            /* istanbul ignore if */
            if (("production" !== 'production') && hot) {
...
idOrOptions, setup, setupOptions) {
    ...
    function useStore(pinia, hot) {
        ...
        pinia =
            ...
            (("production" === 'test') && activePinia && activePinia._testing ? null : pinia) ||
                (currentInstance && inject(piniaSymbol));
        if (pinia)
            setActivePinia(pinia);
        if (("production" !== 'production') && !activePinia) {
...
function mapStores(...stores) {
    if (("production" !== 'production') && Array.isArray(stores[0])) {
...
const versionString = "production" === "development" ? ({}).VITE_APP_VERSION + "-dev" : ({}).VITE_APP_VERSION;
...
const _hoisted_1 = /* @__PURE__ */ createBaseVNode("p", null, "Hello", -1);
const _hoisted_2 = [
  _hoisted_1
];
...
var _Users_uninen_Code_Templates_vite_ts_tailwind_starter_src_components_SimpleComponent_vue = /* @__PURE__ */ _export_sfc(_sfc_main, [["__file", "SimpleComponent.vue"]]);
...
register({ _Users_uninen_Code_Templates_vite_ts_tailwind_starter_src_components_SimpleComponent_vue });
//# sourceMappingURL=index.1d7ad096.js.map
